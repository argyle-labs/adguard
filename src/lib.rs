...
//! (deploy/backup/restore/configure/status/connect/sync) drive adguard. No
...
/// adguard backend. Holds only the provider name; per-instance endpoint/creds
...
impl ServiceBackend for AdguardBackend {
        ...
        -> BoxFuture<'a, Result<BackupArtifact, ServiceError>>
    {
        ...
        Box::pin(async move { Err(ServiceError::unimplemented("adguard.backup")) })
    }

    fn restore<'a>(&'a self, _ep: &'a Endpoint, _from: &'a BackupArtifact)
        -> BoxFuture<'a, Result<(), ServiceError>>
    {
        Box::pin(async move { Err(ServiceError::unimplemented("adguard.restore")) })
    }
